...
class PlaceSearchService {
  ...
  Future<List<MunibPlaceSearchResult>> search(String query) async {
    ...
    debugPrint('[$_tag][PLACE_SEARCH] START query="$value"');
    debugPrint('[$_tag][PLACE_SEARCH] GET $uri');
    ...
    try {
      ...
      debugPrint('[$_tag][PLACE_SEARCH] HTTP ${response.statusCode} bodyBytes=${response.bodyBytes.length}');

      if (response.statusCode != 200) {
        final preview = response.body.length > 1000
            ? response.body.substring(0, 1000)
            : response.body;
        debugPrint('[$_tag][PLACE_SEARCH][ERROR] response=$preview');
        throw Exception('place_search_failed_${response.statusCode}');
      }

      final decoded = jsonDecode(response.body);
      if (decoded is! List) {
        debugPrint('[$_tag][PLACE_SEARCH][ERROR] decoded response is ${decoded.runtimeType}, expected List');
        return const [];
      }
      ...
      debugPrint('[$_tag][PLACE_SEARCH] SUCCESS raw=${decoded.length} filtered=${results.length}');
      if (results.isNotEmpty) {
        final first = results.first;
        debugPrint('[$_tag][PLACE_SEARCH] first=${first.city}, ${first.country} lat=${first.latitude} lon=${first.longitude} cc=${first.countryCode}');
      }
      return results;
    } catch (error, stackTrace) {
      debugPrint('[$_tag][PLACE_SEARCH][EXCEPTION] $error');
      debugPrint('[$_tag][PLACE_SEARCH][STACK] $stackTrace');
      rethrow;
    }
  }
}
