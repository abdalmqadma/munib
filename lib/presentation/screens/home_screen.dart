...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    final screens = <Widget>[
      HomeContent(onStartTraining: () => setState(() => _currentIndex = 2)),
      const AzkarScreen(),
...
class _HomeContentState extends State<HomeContent> {
  ...
  Future<void> _fetchByLocation() async {
    ...
    try {
      ...
      final data = await AIService().fetchPrayerTimesByLocation(city);
      if (data.isEmpty) throw Exception('empty prayer response');

      await provider.setMonthlyPrayers(
        data.map((item) => PrayerDay.fromJson(item)).toList(),
      );
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(context.tr('fetchFailed'))),
        );
      }
    } finally {
...
class _NextPrayerSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final name = _localizedPrayer(context, provider.nextPrayerName);
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.accessibility_new_rounded, color: scheme.primary),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Text(context.tr('teachMePrayer'), style: Theme.of(context).textTheme.titleLarge),
              ),
            ],
          ),
          ...
          SizedBox(
            ...
            child: FilledButton(
              onPressed: onTap,
              child: Text(context.tr('startTraining')),
            ),
          ),
        ],
      ),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.calendar_month_outlined, size: 64, color: scheme.primary),
          const SizedBox(height: 18),
          Text(
            context.tr('noPrayerTimes'),
            textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.headlineSmall,
          ),
          const SizedBox(height: 10),
          Text(
            context.tr('uploadOrLocation'),
            textAlign: TextAlign.center,
